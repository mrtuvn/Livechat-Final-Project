...
/* nicely done ! . New branchs created mrtuvn here <<== */

function message_to_html_newstyle($msg){
	if (isset($kind) && $kind == 'user') return message_to_html_client($msg);
	if(isset($kind) && $kind == 'agent') return message_to_html_agent($msg);
}
function message_to_html_client($msg){
	...
	$msg = { messageid, ikind, created, tname, tmessage};
	...
	*/
	...
	switch($kind) {
		case 'user':
			...
			$message = "<div class=\"live-chat-new-style muser\"><div class=\"messageWrap\"><div class=\"avatar\"><img src=\"http://tu.test/final/images/no-image.jpg\"/></div><div class=\"messageContainer\">";
				
			if ($kind !== 'user'){
				if ($msg['tname'])
					$message .= "<span class=\"nuser\">" . safe_htmlspecialchars($msg['tname']) . "</span>: ";
			}else{
				$message .= "<span class=\"nuser\">me</span>: ";
			}
			$message .= "<span class=\"muser\">" . prepare_html_message($msg['tmessage']) . "</span></div><div class=\"timeSend\"><span>Sent at " . date("H:i:s", $msg['created']) . "</span></div></div><br/>";
			return $message .'</div>';
		...
		case 'agent':
			...
			$message = "<div class=\"live-chat-new-style magent\"><div class=\"messageWrap\"><div class=\"avatar\"><img src=\"http://tu.test/final/images/no-image.jpg\"/></div><div class=\"messageContainer\">";
				
			if ($kind !== 'user'){
				if ($msg['tname'])
					$message .= "<span class=\"nagent\">" . safe_htmlspecialchars($msg['tname']) . "</span>: ";
			}else{
				$message .= "<span class=\"nagent\">me</span>: ";
			}
			$message .= "<span class=\"magent\">" . prepare_html_message($msg['tmessage']) . "</span></div><div class=\"timeSend\"><span>Sent at " . date("H:i:s", $msg['created']) . "</span></div></div><div class=\"istyping\" style=\"display:none;\">Operator is typing...</div><br/>";
			return $message .'</div>';
		...
		case 'event':
			...
			$message = "<div class=\"live-chat-new-style mevent\">";
		...
		case 'conn':
			...
			$message = "<div class=\"live-chat-new-style mconn\">";
				
				
			$message .= "<span class=\"mconn\">" . prepare_html_message($msg['tmessage']) . "</span><br/>";
			return $message .'</div>';
				
			break;

		case 'inf':
			$message = "<div class=\"live-chat-new-style minf\">";

			$message .= "<span class=\"minf\">" . prepare_html_message($msg['tmessage']) . "</span><br/>";
			return $message .'</div>';
	...
	/*
	 $message = "<div class=\"live-chat-new-style m$kind\"><div class=\"messageWrap\"><div class=\"avatar\"><img src=\"http://tu.test/final/images/no-image.jpg\"/></div><div class=\"messageContainer\">";

	if ($kind !== 'user'){
	if ($msg['tname'])
		$message .= "<span class=\"n$kind\">" . safe_htmlspecialchars($msg['tname']) . "</span>: ";
	}else{
	$message .= "<span class=\"n$kind\">me</span>: ";
	}
	$message .= "<span class=\"m$kind\">" . prepare_html_message($msg['tmessage']) . "</span></div><div class=\"timeSend\"><span>" . date("H:i:s", $msg['created']) . "</span></div></div><br/>";
	return $message .'</div>';

	*/
}
function message_to_html_agent($msg){
	global $kind_to_string, $kind_avatar;
	if ($msg['ikind'] == $kind_avatar) return "";

	$kind = $kind_to_string{$msg['ikind']};

	if($kind == 'mevent') { $message ="<div class=\"live-chat-new-note\">" . prepare_html_message($msg['tmessage']) . "</div>"; return $message; }
...
{
	if (isset($newstyle) && $newstyle == 'newstyles') return message_to_html_newstyle($msg); /* customize */
	global $kind_to_string, $kind_avatar;
	if ($msg['ikind'] == $kind_avatar) return "";
...
function print_thread_messages($thread, $token, $lastid, $isuser, $format, $agentid = null, $newstyle=null)
{
	global $mibew_encoding, $mibewroot, $connection_timeout, $settings, $newstyle;
	$newstyle = $newstyle;
